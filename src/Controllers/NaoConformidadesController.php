...
class NaoConformidadesController
{
    ...
    public function excluir($id)
    {
        ...
        try {
            ...
            $isAdmin = isset($_SESSION['user_role']) && in_array($_SESSION['user_role'], ['admin', 'super_admin']);
            if (!$isAdmin) {
                echo json_encode(['success' => false, 'message' => 'Apenas administradores podem excluir NCs']);
                exit;
            }

            // Verificar se NC existe
            $stmt = $this->db->prepare("SELECT id, titulo FROM nao_conformidades WHERE id = ?");
            $stmt->execute([$id]);
            $nc = $stmt->fetch(PDO::FETCH_ASSOC);
            ...
            // Buscar e excluir arquivos anexos do servidor
            $stmt = $this->db->prepare("SELECT caminho_arquivo FROM nao_conformidades_anexos WHERE nc_id = ?");
            $stmt->execute([$id]);
            $anexos = $stmt->fetchAll(PDO::FETCH_ASSOC);
            ...
            $stmt = $this->db->prepare("DELETE FROM nao_conformidades_anexos WHERE nc_id = ?");
            $stmt->execute([$id]);

            // Excluir NC
            $stmt = $this->db->prepare("DELETE FROM nao_conformidades WHERE id = ?");
            $stmt->execute([$id]);

            $this->db->commit();

            error_log("NC #{$id} excluída por user_id: " . $_SESSION['user_id']);

            echo json_encode([
                'success' => true,
                'message' => "NC #{$id} excluída com sucesso!"
            ]);
            ...
            error_log("Erro ao excluir NC: " . $e->getMessage());
            echo json_encode(['success' => false, 'message' => 'Erro ao excluir NC: ' . $e->getMessage()]);
        }
        exit;
    }
