...
class AreaTecnicaController
{
    ...
    private function hasAccess($module = 'area_tecnica')
    {
        ...
        // Para outros usuários, verificar permissão no banco
        return \App\Services\PermissionService::hasPermission($_SESSION['user_id'], $module, 'view');
    }

    // ========================================
    // SISTEMA DE TRIAL (30 dias grátis)
    // ========================================
    ...
    public function ativarTrial()
    {
        ...
        try {
            ...
            echo json_encode(['success' => true, 'message' => 'Trial de 30 dias ativado com sucesso!']);
    ...
    {
        if (!$this->hasAccess()) {
            // Se não está logado, vai para login. Se está logado mas sem permissão, vai para início
            if (!isset($_SESSION['user_id'])) {
                header('Location: /login');
            } else {
                header('Location: /inicio?erro=sem_permissao');
            }
            exit;
        }
    ...
    {
        if (!$this->hasAccess('area_tecnica_consulta')) {
            // Se não está logado, vai para login. Se está logado mas sem permissão, vai para início
            if (!isset($_SESSION['user_id'])) {
                header('Location: /login');
            } else {
