...
// Para NÃO-ADMIN: bloquear se trial não está ativo (independente se é 'nao_iniciado' ou 'expirado')
// Para ADMIN: bloquear apenas se expirado (pode ativar trial)
if ($isAdmin) {
    $moduloBloqueado = $trialInfo['status'] === 'expirado';
} else {
    // Usuário comum: bloqueado se não está ativo (trial não iniciado OU expirado)
  ...
  <div class="flex justify-between items-center">
    ...
    <div id="trialStatus">
      <?php if ($trialInfo['status'] === 'nao_iniciado'): ?>
        ...
        <?php else: ?>
          <!-- Usuário comum vê aviso que precisa aguardar admin -->
          <div class="bg-gradient-to-r from-gray-500 to-slate-600 text-white px-4 py-2 rounded-lg shadow-lg">
            <div class="text-sm font-medium">🔒 Módulo Não Liberado</div>
            <div class="text-xs opacity-90">Aguarde o administrador ativar</div>
          </div>
        <?php endif; ?>
...
async function ativarTrial() {
  if (!confirm('Deseja ativar o período de teste de 30 dias grátis?')) return;
