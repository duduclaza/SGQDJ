            ...
            <form method="POST" class="p-5 space-y-4">
                ...
                <div>
                    ...
                    <input type="text" name="nome_produto" required placeholder="Ex: Servidor, Totem, Coletor..." 
                        class="w-full bg-slate-50 dark:bg-slate-900 border border-slate-300 dark:border-slate-600 rounded-xl px-4 py-2.5 text-sm focus:ring-2 focus:ring-primary-500 transition-all">
                </div>
                ...
                <form method="POST" class="space-y-6">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                            ...
                            <input type="text" name="titulo" required placeholder="Ex: Checklist de Servidores" 
                                class="w-full bg-white dark:bg-slate-900 border border-slate-300 dark:border-slate-600 rounded-xl px-4 py-2.5 text-sm focus:ring-2 focus:ring-primary-500 transition-all">
                        </div>
                        <div>
                            <label class="block text-xs font-semibold text-slate-500 uppercase mb-2">Vincular ao Produto</label>
                            <select name="tipo_produto_id" class="w-full bg-white dark:bg-slate-900 border border-slate-300 dark:border-slate-600 rounded-xl px-4 py-2.5 text-sm focus:ring-2 focus:ring-primary-500 transition-all">
                    ...
                    <div>
                        ...
                        <div id="itensContainer" class="space-y-2">
                            <input type="text" name="itens[]" required placeholder="Item 1" class="w-full bg-white dark:bg-slate-900 border border-slate-300 dark:border-slate-600 rounded-xl px-4 py-2 text-sm">
                        </div>
                    </div>
                        ...
                        <?php foreach ($checklists as $ch): ?>
                            <div class="bg-slate-50/50 dark:bg-slate-900/40 border border-slate-200 dark:border-slate-700 rounded-2xl p-4 flex flex-col justify-between hover:shadow-md transition-all">
                                <div>
                                    ...
                                    <h3 class="font-bold text-slate-800 dark:text-white mb-1"><?= htmlspecialchars($ch['titulo']) ?></h3>
                                    <p class="text-xs text-slate-500">Configurado em: <?= date('d/m/Y', strtotime($ch['criado_em'])) ?></p>
                                </div>
                                <div class="mt-4 pt-4 border-t border-slate-100 dark:border-slate-700 flex justify-between items-center">
                                    <span class="text-xs text-slate-600 dark:text-slate-400">
                                        <i class="ph ph-check-square"></i> Verificação (Sim/Não)
                                    </span>
                                    <span class="badge bg-primary-100 text-primary-700 dark:bg-primary-900/30 dark:text-primary-300 text-[10px] font-bold px-2 py-0.5 rounded">ATIVO</span>
                                </div>
                            </div>
                        <?php endforeach; ?>
                    </div>
...
function addItem() {
    ...
    input.required = true;
    input.className = 'w-full bg-white dark:bg-slate-900 border border-slate-300 dark:border-slate-600 rounded-xl px-4 py-2 text-sm mt-2';
